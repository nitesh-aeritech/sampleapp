...
import 'package:thewarehouse/widgets/statusChange/helper/util.dart';
import 'package:thewarehouse/widgets/statusChange/theme/status_change_theme.dart';
...
class StatusChangeTile extends StatelessWidget {
  const StatusChangeTile({
    ...
    this.crossAxisExtent,
        ...
        assert(
          nodeAlign == StatusChangeNodeAlign.basic || (nodeAlign != StatusChangeNodeAlign.basic && nodePosition == null),
  ...
  double _getEffectiveNodePosition(BuildContext context) {
    ...
    nodePosition ??= (node is StatusChangeTileNode) ? (node as StatusChangeTileNode).getEffectivePosition(context) : .5;
    return nodePosition;
  }
  ...
  Widget build(BuildContext context) {
    ...
    var minNodeExtent = StatusChangeTheme.of(context).indicatorTheme.size ?? 0.0;
    var items = [
      if (nodeFlex > 0)
        Expanded(
          flex: nodeFlex.toInt(),
